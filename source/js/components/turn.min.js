...
(function(f) {
	...
	function P(a, b, c, d, e) {
		...
		if ('-webkit-' == w) {
			...
			a.css({
				'background-image':
					'-webkit-gradient(linear, ' + b.x + '% ' + b.y + '%,' + c.x + '% ' + c.y + '%, ' + f.join(',') + ' )',
			});
		} else {
		...
		g = {
			...
			addPage: function(a, b) {
				...
				1 <= b &&
					...
					(
						...
						d && (e.totalPages = h),
						(e.pageObjs[b] = f(a).css({ float: 'left' }).addClass('page p' + b + c)),
					...
					(
						'double' == b.display &&
							(
								...
								'ltr' == b.direction
									? a[0] ? a[1] || (d += c.width / 4) : (d -= c.width / 4)
									: a[0] ? a[1] || (d -= c.width / 4) : (d += c.width / 4)
							),
			...
			display: function(a) {
				...
				switch (a) {
					...
					case 'double':
						b.pageObjs[0] &&
							(this.turn('stop').css({ overflow: '' }), b.pageObjs[0].remove(), delete b.pageObjs[0]), this.removeClass(
							'shadow'
						);
				}
				b.display = a;
				c &&
					((a = this.turn('size')), g._movePages.call(this, 1, 0), this.turn('size', a.width, a.height).turn('update'));
				return this;
			},
			...
			_turnPage: function(a) {
				...
				'single' == d.display
					? ((b = h[0]), (c = i[0]))
					: h[1] && a > h[1] ? ((b = h[1]), (c = i[0])) : h[0] && a < h[0] && ((b = h[0]), (c = i[1]));
			...
			peel: function(a, b) {
				...
				!1 === a
					...
					: 'single' == c.display
						...
						: (
								(d = 'ltr' == c.direction ? (-1 != a.indexOf('l') ? d[0] : d[1]) : -1 != a.indexOf('l') ? d[1] : d[0]),
			...
			_eventReleased: function(a, b) {
				...
				c =
					'single' == h.display
						? 'br' == b.corner || 'tr' == b.corner ? b.x < c.width() / 2 : b.x > c.width() / 2
						: 0 > b.x || b.x > c.width();
			...
			calculateZ: function(a) {
				...
				for (b = 0; b <= g; b++)
					(c = a[b]), (d = f.pages[c].data().f.opts.next), (e = f.pagePlace[c]), k(c), k(d), (c =
						f.pagePlace[d] == d ? d : c), (j.pageZ[c] = f.totalPages - Math.abs(i - c)), (j.partZ[e] =
						2 * f.totalPages - g + b);
				return j;
			},
			update: function() {
				...
				if (this.turn('animating') && 0 !== b.pageMv[0]) {
					...
					for (a in b.pageWrap)
						if (
							...
							c.flip('z', d.partZ[a] || null), d.pageV[a] && c.flip('resize'), b.tpage
								? c
										.flip('hover', !1)
										.flip('disable', -1 == f.inArray(parseInt(a, 10), b.pageMv) && a != i[0] && a != i[1])
			...
			_updateShadow: function() {
				...
				for (var i = 0; i < c.pageMv.length && a[0] && a[1]; i++)
					(a = this.turn('view', c.pages[c.pageMv[i]].data().f.opts.next)), (b = this.turn(
						'view',
						c.pageMv[i]
		...
		i = {
			...
			_cAllowed: function() {
				...
				return 'hard' == a.effect
					? 'ltr' == c.direction ? [d ? 'r' : 'l'] : [d ? 'l' : 'r']
					: 'single' == c.display
						? 1 == b
							? 'ltr' == c.direction ? p.forward : p.backward
							: b == c.totalPages ? ('ltr' == c.direction ? p.backward : p.forward) : p.all
						: 'ltr' == c.direction ? p[d ? 'forward' : 'backward'] : p[d ? 'backward' : 'forward'];
			},
			...
			_foldingPage: function() {
				...
				if (a) {
					var b = a.opts;
					if (b.turn)
						return (a = b.turn.data()), 'single' == a.display
							? 1 < b.next || 1 < b.page ? a.pageObjs[0] : null
							: a.pageObjs[b.next];
				}
			},
			_backGradient: function() {
				var a = this.data().f,
					b = a.opts.turn.data();
				if (
					(b = b.opts.gradients && ('single' == b.display || (2 != a.opts.page && a.opts.page != b.totalPages - 1))) &&
					!a.bshadow
				)
					...
					case 'hard':
						a &&
							(
								...
								c.opts.gradients && (b.ashadow.css({ width: d, height: e }), b.bshadow.css({ width: d, height: e }))
							);
						break;
					case 'sheet':
						a &&
				...
				if (!a.wrapper)
					switch (a.effect) {
						case 'hard':
							...
							b.opts.gradients &&
								((a.ashadow = f('<div/>', l(0, 0, 0)).hide().appendTo(c)), (a.bshadow = f('<div/>', l(0, 0, 0))));
							break;
						case 'sheet':
							var d = this.width(),
								e = this.height();
								...
								c.opts.gradients &&
									(
										...
										i._backGradient.call(u) &&
											(
												(B = j(100 * (1.2 * y * Math.sin(g) / e), 100 * (1.2 * y * Math.cos(g) / h))),
												C || (B.x = 100 - B.x),
												E || (B.y = 100 - B.y)
											)
									);
								...
								(
									...
									i._backGradient.call(u) &&
										P(
											...
											j(C ? 0 : 100, E ? 0 : 100),
											j(B.x, B.y),
											[[0.6, 'rgba(0,0,0,0)'], [0.8, 'rgba(0,0,0,' + 0.3 * N + ')'], [1, 'rgba(0,0,0,0)']],
											3
										)
								);
						};
			...
			_showFoldedPage: function(a, b) {
				...
				if (c) {
					if (!f || !e.point || e.point.corner != a.corner)
						if (
							(
								(c = 'hover' == e.status || 'peel' == e.status || e.opts.turn.data().mouseAction ? a.corner : null),
								(f = !1),
					...
					if (!f)
						switch (e.effect) {
							case 'hard':
								...
								break;
							case 'sheet':
								(e.visible = !0), e.fparent.show().data().flips++, i._moveFoldingPage.call(
									this,
									!0
					...
					if (((a = u ? a.originalEvent.touches : [a]), b.corner)) {
						...
						if ((a = i._isIArea.call(this, a[0]))) {
							if (('sheet' == b.effect && 2 == a.corner.length) || 'hard' == b.effect)
								(b.status = 'hover'), (b = i._c.call(this, a.corner, b.opts.cornerSize / 2)), (a.x = b.x), (a.y =
									b.y), i._showFoldedPage.call(this, a, !0);
